...
export default function CompleteTask() {
    const { data: completeData, isLoading, error } = useAllOfferQuery('Done');
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 mb-6">
                ...
                <div className="flex items-center gap-4 text-sm sm:text-base">
                    <span className="font-semibold text-[#059669]">
                        {completeData?.total_offers || 0} tasks
                    </span>
                    <span className="text-[#6B7280]">
                        Total: ${completeData?.total_price?.toLocaleString() || '0'}
                    </span>
                </div>
            </div>
            {tasks.length === 0 ? (
