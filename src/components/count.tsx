...
export default function Count() {
  ...
  const { data: pendingData } = useAllOfferQuery('Pending');
  const { data: acceptedData } = useAllOfferQuery('Accepted');
  const { data: doneData } = useAllOfferQuery('Done');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-10">
        {cardData.map((card) => (
          <div
            ...
            className={`${
              active === card.key ? selectedBg : "bg-white"
            } cursor-pointer flex items-center gap-4 rounded-xl shadow-md px-4 py-6 hover:shadow-lg transition-all duration-300 border-2 ${
              active === card.key 
                ? 'border-[#059669]' 
