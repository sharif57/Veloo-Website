...
export default function Projects() {
  ...
  const tabsAndComponents = [
    ...
    { id: 2, label: 'Quote', content: <div className="p-4">Quote Content</div> },
    { id: 3, label: 'Communication', content: <div className="p-4">Communication Content</div> },
    { id: 4, label: 'Payment', content: <div className="p-4">Payment Content</div> },
    { id: 5, label: 'Materials', content: <div className="p-4">Materials Content</div> },
  ];

  return (
    <div className="bg-white p-4 md:p-6 rounded-lg shadow-md mx-auto container h-screen">
        ...
        <ul className="flex flex-col md:flex-row items-center gap-2 md:gap-5 overflow-x-auto pb-2">
          {tabsAndComponents.map((tab) => (
            <li
              key={tab.id}
              className={`${
                isActive === tab.id && '!border-[#059669] !text-[#059669]'
